= Personal Tips & Tricks (2026)
Pradeep Roy
:source-highlighter: rouge
:source-language: console

These are a collection of mainly software usage tips that I have noted down
over several years. My working environment is mostly Ubuntu 24.04, but also
includes Debian 12, Oracle Linux 9 and Rocky Linux 8. Earlier, it used to be
CentOS 7.
...
The comment helps identify the key or its role. An easy first choice is the
system's hostname. When multiple keys are required, the `-f` switch makes
it possible to explicitly supply a pathname. Example:

----
$ ssh-keygen -t rsa -b 4096 -C ubuntu-wsl -f ~/.ssh/id_rsa_wsl
----
...
== Rsync - Including Files

When working with a lot of data, one is sometimes interested in statistics
(e.g. inside CSV files). Rsync can be used to preserve the directory hierarchy
and fetch certain files. Example:
...
The order matters because `rsync` uses the first matching rule:
`--include='*/'` includes all directories, `--include='*.csv'` selects CSV
files, and `--exclude='*'` prevents all other files from being copied. Without
the final exclude, `rsync` would copy unmatched files because its default
behavior is to transfer everything.

== VirtualBox - Guest Additions

I tried these on an Ubuntu 24.04 server guest with Windows 11 host and
VirtualBox https://www.virtualbox.org/wiki/Changelog[7.2.6]. Installation of
https://www.virtualbox.org/manual/topics/guestadditions.html[guest additions]
improves performance and usability.

Ensure that the guest additions ISO is mounted on the VM. I found the ISO in
the `C:\Program Files\Oracle\VirtualBox` folder. The boot-up the machine.

Check if the CD-ROM device was detected:

----
$ sudo blkid | grep sr0
----

I got an output that looked like the following

----
/dev/sr0: BLOCK_SIZE="2048" UUID="..." LABEL="VBox_GAs_7.2.6" TYPE="iso9660"
----
...
$ sudo mount -oro /dev/sr0 /mnt/cdrom
----

Prepare the guest system for building external kernel modules
...
----
VirtualBox Guest Additions: kernel modules 7.2.6 r172322 are loaded
----
...
I was searching for a tool that could improve the viewing experience of CSV
files. For instance, I wanted to have values for the same column aligned
together. This is exactly the kind of work that can be done by
https://miller.readthedocs.io/en/6.17.0/[Miller]. It is a very useful tool to
query and reformat data from text files in various formats.

It can be installed on Ubuntu 22.04 by running:
...
----
$ mlr -C --key-color mediumturquoise --value-color silver --icsv --opprint cut -f 'Date,Close' <file>
----

Miller has a large number of features and is also well documented. For
information, look at its man page:
...
Installing Go on a Linux distribution running on an Intel 64-bit processor is
very easy.

----
$ curl -sSL -O https://golang.org/dl/go1.24.9.linux-amd64.tar.gz
$ sudo tar -C /usr/local -xf go1.24.9.linux-amd64.tar.gz
----

Finally, do ensure that `/usr/local/go/bin` is in the `PATH` environment
variable. I am not writing an exact command because this might depend on user
preference.
...
----
$ curl -sSfL https://raw.githubusercontent.com/golangci/golangci-lint/HEAD/install.sh | sh -s -- -b ~/.local/bin/ v2.5.0
----

To find the specific version, visit the
https://github.com/golangci/golangci-lint/releases[releases] page. Not
providing a version results in downloading the latest release.
...
Observe the use of `--delete` to delete a remote branche. An alternative form
is:
...
I have been using https://www.vim.org/about.php[Vim] since a long time and it
has been a practice to use C-] and C-t to navigate source code. It started
with `ctags` for C/C++ and smoothly transitioned to
https://github.com/fatih/vim-go[vim-go] for Go.

Now (2026) I wanted the same user experience with Python. The ensuing steps
are about configuring Neovim for navigating through Python sources.  I could
have explored Vim but thought of trying something else so that the existing
editor settings did not get disturbed.  Another reason for choosing Neovim was
that it had an built-in Language Server Protocol (LSP) client.

These have been tested on Debian 12.

=== Debian 12

Various tools had these versions:

- *npm* 9.2.0 This was from Ubuntu
- *Pyright* 1.1.409 This was installed using npm
- *Neovim* v0.12.2 This was installed using the `curl` command that downloaded
  the latest amd64 binaries.
- *lsp-config* Latest `master` branch with commit `229b7905` from 03-Jun-2026.
  The closest tag was v2.9.0.

Install https://microsoft.github.io/pyright/[Pyright] - static code analyzer
...
Ensure that the `nvim` executable is in a directory referenced by `PATH`.
Example:
...
Install plugin for Neovim LSP client configuration:

----
$ mkdir -p ~/.config/nvim/pack/core/start && git clone https://github.com/neovim/nvim-lspconfig ~/.config/nvim/pack/core/start/nvim-lspconfig
----
...
The setup did not work due to nvim-lspconfig incompatibility with Neovim.
Presumably, the plugin was much newer than the editor. Through, hit-and-trial
a working tag from December 2023 was found:

----
$ cd ~/.config/nvim/pack/core/start/nvim-lspconfig
$ git checkout v0.1.7
----
...
- *npm* 10.9.7 This was from Ubuntu
- *Pyright* 1.1.408 This was installed using npm
- *Neovim* v0.12.1 This was installed using the `curl` command that downloaded
  the latest amd64 binaries.
- *lsp-config* Latest `master` branch with commit `bedca8b4` from 06-Apr-2026.
  The closest tag was v2.7.0.

I had to change `init.lua` slightly:
...
These steps made it possible to jump to symbol definitions in Python code.
...
== Shell - Fractional Exponent

I often make use of the `bc` command for calculations. It generally cannot
deal with a fractional exponent when computing powers (e.g. `2^0.5`). This
limitation can be circumvented using the `-l` switch that asks `bc` to make
certain useful functions available. Two functions of interest are:

- *l(x)* The natural logarithm of x
- *e(x)* The exponential function of raising e to the value x

These two functions don't have a problem with fractional arguments. This
allows us to compute `a^b` by using `e(b * l(a))`. Example:
...
This capability is necessary when finding CAGR (Compound Annual Growth Rate)
values. Let us say some quantity had the value 31497 on 03-Oct-2017 and 85836
on 26-Sep-2024. The number of days between these two dates is 2549 or 6.9835
years.

----
$ bc -q -l
e((1 / 6.9835) * l(85836 / 31497))
1.15437523416471384176
----

This means a CAGR value of 15.43%.
...
Once I was working inside a minimal Docker container to create a build
environment. The `rpmbuild` command did not exist. A DNF command that can
search for packages containing a specific file helped:

----
$ dnf provides rpmbuild
----

The output pointed to the package `rpm-build`:

----
$ dnf install rpm-build
----

== Packages - Available Version (APT)
...
== Sumatra PDF - Viewing PDF Files

Continuous and silent connectivity to the Internet became a reality for home
computers in the late 2000s. Earlier, users needed to take deliberate action
to connect to another computer. The situation has dramatically changed in the
last 20 years. Now applications receive inputs over the Internet and talk to
users (displaying ads) indiscriminately. This unwanted communication disrupts
user attention and lowers the quality of the actual work that was supposed to
be enabled by the application. Unfortunately, monetary payments are no longer
a reliable protection from interruptions. Silence has been replaced by
constant upselling.

I installed a well-known PDF reader on Windows 10. The application designers
decided that it was okay to split the screen into two parts. One was for the
viewing of PDF files and the other was to display ads. Even when ads were not
being flashed, I could not use the full screen to read documents.

https://www.sumatrapdfreader.org/free-pdf-reader[Sumatra PDF] makes it
possible to read PDF documents without distraction on Windows. It has the good
behaviour of earlier applications. Install and forget. It lets the user focus
on the end goal of gaining knowledge.

== QPDF - Encrypting PDF files

The `qpdf` command can be used to encrypt a PDF file. This also ensures that a
password is required to open it. Install on a Debian-like system:

----
$ sudo apt-get install qpdf
----

Encrypt a file writing output to a separate file:

----
$ qpdf --encrypt <user_password> <owner_password> 256 -- <in.pdf> <out.pdf>
----

To view the encryption status of a file:

----
$ qpdf --show-encryption <out.pdf>
----

If one does not want a new file to be created:

----
$ qpdf --replace-input --encrypt <user_password> <owner_password> 256 -- <in.pdf>
----

For more details:

----
$ qpdf --help=encryption | less
----
...
This method worked on WSL 2.7.3.0 and Debian 12.12. To change the hostname
inside WSL make use of
https://learn.microsoft.com/en-us/windows/wsl/wsl-config#wslconf[wsl.conf].
Example:
...
Instructions in this section worked with Node.js 22.23.2, npm 10.9.8 and codex
0.153.0.
...
Install relevant packages necessary for sandboxing:
...
Stock Rocky Linux 8 packages Node.js 10.24.0 and npm 6.14.11. This was not
sufficient for Codex 0.153.0.

Install https://nodesource.com/products/distributions[Node.js]:
...
Following these steps installed Node.js 22.23.2 and npm 10.9.8. Then, install
the sandboxing tool:
...
If sandboxing everything is setup correctly (on Ubuntu 24.04 or Rocky Linux 8)
then the following command should return an exit status of zero:

----
$ bwrap --unshare-user --unshare-pid --ro-bind / / --proc /proc --dev /dev /usr/bin/true
----

== Windows - Disk Cleanup

I was able to try these teps on Windows 11 25H2. The disk cleanup program can
be accessed by pressing Win + R and running `cleanmgr`. Once, it was able to
reclaim at much as 10 GB from the C drive.
